...
const parse_string = (date) => date.toLocaleDateString().replaceAll(".", "").replaceAll(" ", "")

let today = parse_string(new Date())

let yesterDay = new Date()
yesterDay.setTime(new Date().getTime() - (1 * 24 * 60 * 60 * 1000)) // 몇일 전으로 돌리고 싶으면 * 일수를 해주면 됨.
...
queryParams += '&' + encodeURIComponent('startCreateDt') + '=' + encodeURIComponent(yesterDay); /* */
...
function ApiCall() {
    return request({
        ...
        try {
            ...
            //     console.log(arr[i].stateDt._text, "일 확진자 : ",arr[i].decideCnt._text - arr[i+1].decideCnt._text)
            // }

            // 당일 확진자 수만 궁금할 때 :: 2021-03-30 dongwon
            console.log(arr[0].stateDt._text, "일 확진자 : ",arr[0].decideCnt._text - arr[1].decideCnt._text)
            console.log("-------------------")
        } catch (error) {
            console.log(error)
        }
